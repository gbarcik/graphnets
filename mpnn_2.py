...
class MPNN(nn.Module):
    ...
    def __init__(self, in_feats, hidden_feats, edge_feats, out_feats, useCuda=False):
        ...
        self.encoder = nn.Linear(in_feats + hidden_feats +1, hidden_feats)
        self.M = nn.Linear( hidden_feats * 2 + edge_feats, 32)
        self.U = nn.Linear(hidden_feats * 2 , hidden_feats)
        self.decoder = nn.Linear(hidden_feats * 2 , in_feats)
        self.termination = nn.Linear(hidden_feats , 1)
    ...
    def predict(self, graph, states, edges_mat):
        ...
        # Clone to avoid modifying ground truth data
        ...
        for _ in range(graph.number_of_nodes()) :
            ...
            softmax, hidden, stop = self.step(graph, new_state, hidden)
